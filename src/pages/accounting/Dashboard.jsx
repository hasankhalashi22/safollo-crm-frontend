...
export default function AccountingDashboard() {
  ...
  const BRAND_STYLES = {
    'bKash': { bg: 'bg-pink-50', border: 'border-pink-100', text: 'text-pink-600', badge: 'bg-pink-500', icon: Smartphone },
    'Rocket': { bg: 'bg-purple-50', border: 'border-purple-100', text: 'text-purple-600', badge: 'bg-purple-600', icon: Rocket },
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-6 gap-3">
        ...
        {(() => { const Icon = BRAND_STYLES['bKash'].icon; return (
          <div className={`card ${BRAND_STYLES['bKash'].bg} border ${BRAND_STYLES['bKash'].border} relative`}>
            <button type="button" onClick={() => setRateModal('bkash')} className="absolute top-3 right-3 p-1 bg-white/60 rounded-full z-10">
              <Pencil size={12} className="text-gray-400" />
            </button>
            <div className={`w-8 h-8 rounded-full ${BRAND_STYLES['bKash'].badge} text-white flex items-center justify-center mb-2`}>
              <Icon size={16} />
            </div>
            <p className="text-sm text-gray-500 mb-1">Today's bKash</p>
            <p className={`text-xl font-bold ${BRAND_STYLES['bKash'].text}`}>Tk {Number(data.today_bkash).toLocaleString()}</p>
            <p className="text-xs text-gray-400 mt-1">Charge: {settings.bkash_charge_rate ?? '—'}%</p>
          </div>
        ); })()}

        {/* Rocket collection */}
        {(() => { const Icon = BRAND_STYLES['Rocket'].icon; return (
          <div className={`card ${BRAND_STYLES['Rocket'].bg} border ${BRAND_STYLES['Rocket'].border} relative`}>
            <button type="button" onClick={() => setRateModal('rocket')} className="absolute top-3 right-3 p-1 bg-white/60 rounded-full z-10">
              <Pencil size={12} className="text-gray-400" />
            </button>
            <div className={`w-8 h-8 rounded-full ${BRAND_STYLES['Rocket'].badge} text-white flex items-center justify-center mb-2`}>
              <Icon size={16} />
            </div>
            <p className="text-sm text-gray-500 mb-1">Today's Rocket</p>
            <p className={`text-xl font-bold ${BRAND_STYLES['Rocket'].text}`}>Tk {Number(data.today_rocket).toLocaleString()}</p>
            <p className="text-xs text-gray-400 mt-1">Charge: {settings.rocket_charge_rate ?? '—'}%</p>
          </div>
        ); })()}
        ...
        {highlightAccounts.map(a => {
          ...
          return (
            <div key={a.id} className={`card ${style.bg} border ${style.border}`}>
              ...
              <p className={`text-xl font-bold ${a.balance < 0 ? 'text-red-500' : style.text}`}>Tk {Number(a.balance).toLocaleString()}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <StatCard icon={<TrendingDown className="text-red-500" size={20} />} label="This Month's Expense" value={summary.month_out} bg="bg-red-50" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="card bg-primary-50">
          ...
          <p className="text-2xl font-bold text-primary-600">Tk {Number(total_assets).toLocaleString()}</p>
        </div>
        <div className="card bg-orange-50">
          <p className="text-sm text-gray-500 mb-1">Total Liabilities</p>
          <p className="text-2xl font-bold text-orange-600">Tk {Number(total_liabilities).toLocaleString()}</p>
        </div>
      </div>

      {/* Asset Accounts */}
      <div className="card">
        <h3 className="font-semibold text-dark mb-3 flex items-center gap-2"><Wallet size={18} /> Account Balances</h3>
        <div className="space-y-2">
          {assets.map(a => (
            <div key={a.id} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
              ...
              <p className={`font-semibold ${a.balance < 0 ? 'text-red-500' : 'text-dark'}`}>Tk {Number(a.balance).toLocaleString()}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {liabilities.length > 0 && (
        <div className="card">
          <h3 className="font-semibold text-dark mb-3 flex items-center gap-2"><CreditCard size={18} /> Liabilities</h3>
          <div className="space-y-2">
            {liabilities.map(a => (
              <div key={a.id} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                <div>
                ...
                <p className="font-semibold text-orange-600">Tk {Number(a.balance).toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Recent Transactions */}
      <div className="card">
        <h3 className="font-semibold text-dark mb-3">Recent Entries</h3>
        {recent_transactions.length === 0 ? (
          <p className="text-center py-6 text-gray-400 text-sm">No entries yet</p>
        ) : (
          <div className="space-y-2">
            {recent_transactions.map(t => (
              <div key={t.id} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                ...
                <p className={`font-semibold ${t.transaction_type === 'revenue' ? 'text-green-600' : t.transaction_type === 'expense' ? 'text-red-600' : 'text-blue-600'}`}>
                  Tk {Number(t.amount).toLocaleString()}
                </p>
              </div>
            ))}
          </div>
...
function StatCard({ icon, label, value, bg }) {
  return (
    <div className={`${bg} rounded-2xl p-3.5`}>
      ...
      <p className="text-xl font-bold text-dark">Tk {Number(value).toLocaleString()}</p>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-5">
        <div className="flex justify-between mb-4">
          <h3 className="font-bold text-lg">{type === 'bkash' ? 'bKash' : 'Rocket'} Charge Rate</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input type="number" step="0.01" className="input-field" value={rate}
              onChange={e => setRate(e.target.value)} placeholder="e.g. 1.15" />
          </div>
