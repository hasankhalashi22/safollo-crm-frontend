...
import { Plus, ArrowLeft, Edit2, Trash2, CheckCircle, Clock, Save, X } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md p-6 space-y-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea className="input-field" rows={3} value={note} onChange={e => setNote(e.target.value)} placeholder="ক্লাস সম্পর্কে মন্তব্য..." />
        </div>
...
function OutlineRow({ row, teachers, zooms, onRefresh, onFeedback }) {
  ...
  return (
    <tr className="border-t border-gray-100 hover:bg-gray-50/50 group">
      ...
      {editing ? (
        <>
          <td className="px-2 py-2"><input type="date" className="input-field text-xs py-1.5" value={form.scheduled_date} onChange={e => set('scheduled_date', e.target.value)} /></td>
          <td className="px-2 py-2"><input type="time" className="input-field text-xs py-1.5" value={form.scheduled_time} onChange={e => set('scheduled_time', e.target.value)} /></td>
          <td className="px-2 py-2"><input className="input-field text-xs py-1.5" value={form.topic} onChange={e => set('topic', e.target.value)} placeholder="বিষয়" /></td>
          <td className="px-2 py-2">
            <select className="input-field text-xs py-1.5" value={form.teacher_id} onChange={e => set('teacher_id', e.target.value)}>
              <option value="">—</option>
              {teachers.map(t => <option key={t.id} value={t.id}>{t.full_name}</option>)}
            </select>
          </td>
          <td className="px-2 py-2">
            <select className="input-field text-xs py-1.5" value={form.zoom_account_id} onChange={e => set('zoom_account_id', e.target.value)}>
              <option value="">—</option>
              {zooms.map(z => <option key={z.id} value={z.id}>{z.account_name}</option>)}
            </select>
          </td>
          <td className="px-2 py-2">
            <select className="input-field text-xs py-1.5" value={form.class_type} onChange={e => set('class_type', e.target.value)}>
              {Object.entries(CLASS_TYPE_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
          </td>
          <td className="px-2 py-2">
            <select className="input-field text-xs py-1.5" value={form.status} onChange={e => set('status', e.target.value)}>
              {Object.entries(STATUS_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
          </td>
          <td className="px-2 py-2">
            <div className="flex gap-1">
              <button onClick={save} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg"><Save size={14} /></button>
              <button onClick={() => setEditing(false)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg"><X size={14} /></button>
            </div>
          </td>
        </>
      ) : (
        <>
          ...
          <td className="px-4 py-3 text-sm font-medium">{row.topic || '—'}</td>
          <td className="px-4 py-3 text-xs text-gray-500">{teacher?.full_name || '—'}</td>
          <td className="px-4 py-3 text-xs text-gray-500">{zoom?.account_name || '—'}</td>
          ...
          <td className="px-4 py-3">
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {row.status === 'scheduled' && row.row_type === 'class' && (
                <button onClick={() => onFeedback(row)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg" title="ফিডব্যাক"><CheckCircle size={14} /></button>
              )}
              <button onClick={() => setEditing(true)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 size={14} /></button>
              <button onClick={del} className="p-1.5 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>
            </div>
          </td>
        </>
      )}
...
export default function BatchDetail() {
  ...
  const [addForm, setAddForm] = useState({ row_type: 'class', scheduled_date: '', scheduled_time: '', topic: '', teacher_id: '', zoom_account_id: '', class_type: 'regular' });
  ...
  const addRow = async () => {
    try {
      ...
      setAddForm({ row_type: 'class', scheduled_date: '', scheduled_time: '', topic: '', teacher_id: '', zoom_account_id: '', class_type: 'regular' });
      loadOutline();
    } catch { toast.error('সমস্যা হয়েছে'); }
  };
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex justify-end">
        <button onClick={() => setShowAddForm(s => !s)} className="bg-primary-500 hover:bg-primary-600 text-white font-semibold px-5 py-2.5 rounded-xl flex items-center gap-2 text-sm transition-colors">
      ...
      {/* Add form */}
      {showAddForm && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border-2 border-primary-200 space-y-4">
          <div className="flex items-center gap-2">
            <div className="w-1 h-5 bg-primary-500 rounded-full"></div>
            <p className="text-sm font-semibold text-gray-700">নতুন সারি যোগ করুন</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-600">বিষয়/শিরোনাম</label>
              <input className="input-field text-sm" placeholder="যেমন: বাংলা ব্যাকরণ" value={addForm.topic} onChange={e => set('topic', e.target.value)} />
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            {addForm.row_type === 'class' && (
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-gray-600">ক্লাস ধরন</label>
                <select className="input-field text-sm" value={addForm.class_type} onChange={e => set('class_type', e.target.value)}>
                  {Object.entries(CLASS_TYPE_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                </select>
              </div>
            )}
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={addRow} className="bg-primary-500 hover:bg-primary-600 text-white font-semibold px-8 py-2.5 rounded-xl transition-colors text-sm">যোগ করুন</button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs">
                <tr>
                  {['#', 'ধরন', 'তারিখ', 'সময়', 'বিষয়', 'শিক্ষক', 'Zoom', 'ক্লাস ধরন', 'অবস্থা', ''].map((h, i) => (
