...
import { Plus, ArrowLeft, Edit2, Trash2, CheckCircle, Clock, Save, X, Wand2 } from 'lucide-react';
...
function routineStatus(row) {
  ...
  if (row.scheduled_date?.split('T')[0] === TODAY) return { label: 'আজ', cls: 'bg-blue-100 text-blue-700' };
...
// ── Existing Outline Row (saved rows) ─────────────────────────────────────────
function OutlineRow({ row, teachers, zooms, onRefresh, onFeedback }) {
  ...
  return (
    <tr className="border-t border-gray-100 hover:bg-gray-50/50 group">
      <td className="px-3 py-2.5 text-xs text-gray-400 w-8">{row.row_no}</td>
      ...
      {editing ? (
        <>
          <td className="px-2 py-1.5"><input type="date" className="input-field text-xs py-1" value={form.scheduled_date} onChange={e => set('scheduled_date', e.target.value)} /></td>
          <td className="px-2 py-1.5"><input type="time" className="input-field text-xs py-1" value={form.scheduled_time} onChange={e => set('scheduled_time', e.target.value)} /></td>
          <td className="px-2 py-1.5"><input className="input-field text-xs py-1" value={form.subject_name} onChange={e => set('subject_name', e.target.value)} placeholder="সাবজেক্ট" /></td>
          <td className="px-2 py-1.5"><input className="input-field text-xs py-1" value={form.topic} onChange={e => set('topic', e.target.value)} placeholder="শিরোনাম" /></td>
          ...
          <td className="px-2 py-1.5"><input className="input-field text-xs py-1" value={form.location} onChange={e => set('location', e.target.value)} placeholder="স্থান" /></td>
          <td className="px-2 py-1.5">
        ...
        <>
          <td className="px-3 py-2.5 text-xs text-gray-600">{row.scheduled_date ? new Date(row.scheduled_date).toLocaleDateString('bn-BD') : '—'}</td>
          <td className="px-3 py-2.5 text-xs text-gray-500">{row.scheduled_time || '—'}</td>
          <td className="px-3 py-2.5 text-xs text-primary-600 font-medium">{row.subject_name || '—'}</td>
          <td className="px-3 py-2.5 text-sm font-medium">{row.topic || '—'}</td>
          <td className="px-3 py-2.5 text-xs text-gray-500">{teacher?.full_name || row.teacher_name || '—'}</td>
          <td className="px-3 py-2.5 text-xs text-gray-500">{zoom?.account_name || row.zoom_account_name || '—'}</td>
          <td className="px-3 py-2.5 text-xs text-gray-500">{row.class_mode === 'offline' ? 'অফলাইন' : 'অনলাইন'}</td>
          <td className="px-3 py-2.5 text-xs text-gray-500">{row.location || '—'}</td>
          <td className="px-3 py-2.5">
          ...
          <td className="px-3 py-2.5">
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button onClick={() => setEditing(true)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 size={13} /></button>
              <button onClick={del} className="p-1.5 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={13} /></button>
            </div>
          </td>
        </>
      )}
...
// ── Generated Row (preview, editable inline) ───────────────────────────────────
function GenRow({ row, idx, teachers, zooms, onChange, onDelete }) {
  const st = routineStatus(row);
  ...
  return (
    <tr className={`border-t border-gray-100 ${isExam ? 'bg-purple-50/40' : ''}`}>
      <td className="px-2 py-1.5 text-center text-xs text-gray-500">{isExam ? '—' : row.class_no}</td>
      <td className="px-2 py-1.5 text-center text-xs text-gray-500">{isExam ? row.exam_no : '—'}</td>
      <td className="px-2 py-1.5">
        <input type="date" className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.scheduled_date} onChange={e => set('scheduled_date', e.target.value)} />
      </td>
      <td className="px-2 py-1.5 text-xs text-center text-gray-500 w-10">{dayLabel}</td>
      <td className="px-2 py-1.5">
        <input type="time" className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.scheduled_time} onChange={e => set('scheduled_time', e.target.value)} />
      </td>
      <td className="px-2 py-1.5">
        <input className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.subject_name} onChange={e => set('subject_name', e.target.value)} placeholder="সাবজেক্ট" />
      </td>
      <td className="px-2 py-1.5">
        <input className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.topic} onChange={e => set('topic', e.target.value)} placeholder="শিরোনাম" />
      </td>
      <td className="px-2 py-1.5">
        <input className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.notes} onChange={e => set('notes', e.target.value)} placeholder="বিস্তারিত" />
      </td>
      <td className="px-2 py-1.5">
        <select className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.zoom_account_id} onChange={e => set('zoom_account_id', e.target.value)} disabled={isExam}>
          <option value="">—</option>
          {zooms.map(z => <option key={z.id} value={z.id}>{z.account_name}</option>)}
        </select>
      </td>
      <td className="px-2 py-1.5">
        <select className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.teacher_id} onChange={e => set('teacher_id', e.target.value)} disabled={isExam}>
          <option value="">—</option>
          {teachers.map(t => <option key={t.id} value={t.id}>{t.full_name}</option>)}
        </select>
      </td>
      <td className="px-2 py-1.5">
        <select className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.class_mode} onChange={e => set('class_mode', e.target.value)}>
          <option value="online">অনলাইন</option>
          <option value="offline">অফলাইন</option>
        </select>
      </td>
      <td className="px-2 py-1.5">
        <input className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-400"
          value={row.location} onChange={e => set('location', e.target.value)} placeholder="স্থান" />
      </td>
      <td className="px-2 py-1.5 text-center">
        <span className={`text-xs px-2 py-0.5 rounded-full ${st.cls}`}>{st.label}</span>
      </td>
      <td className="px-2 py-1.5">
        <button onClick={() => onDelete(idx)} className="p-1 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={12} /></button>
      </td>
    </tr>
  );
}
...
function AutoRoutineGenerator({ batch, teachers, zooms, onSaved }) {
  ...
  const generate = () => {
    ...
    let examNo = 1;
    let subjIdx = 0;
    let cur = new Date(cfg.startDate + 'T00:00:00');

    while (classNo <= Number(cfg.totalClasses)) {
      // advance to next selected class day
      while (!cfg.days.includes(cur.getDay())) {
      ...
      // exam next calendar day
      const examDate = new Date(cur.getTime() + 86400000);
      rows.push({
        _id: `gen-${rows.length}`,
        row_type: 'exam',
        class_no: null,
      ...
      // next iteration starts after exam day
      cur = new Date(examDate.getTime() + 86400000);
    }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border-2 border-primary-100 p-5 space-y-5">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-gray-700 flex items-center gap-2"><Wand2 size={15} className="text-primary-500" /> অটো রুটিন কনফিগারেশন</p>
        {subjects.length > 0 && <span className="ml-auto text-xs text-gray-400">{subjects.length}টি সাবজেক্ট পাওয়া গেছে</span>}
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-gray-600">শুরুর তারিখ</label>
          <input type="date" className="input-field text-sm" value={cfg.startDate} onChange={e => setC('startDate', e.target.value)} />
        </div>
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-gray-600">মোট ক্লাস সংখ্যা</label>
          <input type="number" className="input-field text-sm" min="1" max="200" value={cfg.totalClasses} onChange={e => setC('totalClasses', e.target.value)} />
        </div>
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-1 flex-wrap">
            {DAY_LABELS.map((d, i) => (
              <button key={i} type="button" onClick={() => toggleDay(i)}
                className={`text-xs px-2 py-1 rounded-lg border transition-colors ${cfg.days.includes(i) ? 'bg-primary-500 text-white border-primary-500' : 'bg-white text-gray-500 border-gray-200 hover:border-primary-300'}`}>
                {d}
              </button>
            ))}
          </div>
      ...
      {generatedRows.length > 0 && (
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          <div className="bg-amber-50 px-4 py-2 text-xs text-amber-700 font-medium border-b border-amber-100">
            প্রিভিউ — সেভ করার আগে যেকোনো ঘর সম্পাদনা করা যাবে
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm" style={{ minWidth: '1200px' }}>
              <colgroup>
                <col style={{width:46}} /><col style={{width:52}} /><col style={{width:100}} />
                <col style={{width:40}} /><col style={{width:88}} /><col style={{width:110}} />
                <col style={{width:130}} /><col style={{width:120}} /><col style={{width:110}} />
                <col style={{width:100}} /><col style={{width:80}} /><col style={{width:90}} />
                <col style={{width:72}} /><col style={{width:40}} />
              </colgroup>
              <thead className="bg-gray-50 text-gray-500">
                <tr>
                  {['ক্লাস ক্রম','পরীক্ষা ক্রম','তারিখ','বার','সময়','সাবজেক্ট নাম','শিরোনাম','বিস্তারিত','জুম একাউন্ট','টিচার নাম','ক্লাসের ধরণ','স্থান','ক্লাস স্ট্যাটাস',''].map((h, i) => (
              ...
              <tbody>
                {generatedRows.map((row, idx) => (
                  <GenRow key={row._id} row={row} idx={idx} teachers={teachers} zooms={zooms} onChange={updateRow} onDelete={deleteRow} />
                ))}
              </tbody>
            </table>
          </div>
...
export default function BatchDetail() {
  ...
  const [addForm, setAddForm] = useState({
    ...
    teacher_id: '', zoom_account_id: '', class_type: 'regular', class_mode: 'online', location: '',
  });
  ...
  const addRow = async () => {
    try {
      ...
      setAddForm({ row_type: 'class', scheduled_date: '', scheduled_time: '', topic: '', subject_name: '', teacher_id: '', zoom_account_id: '', class_type: 'regular', class_mode: 'online', location: '' });
      loadOutline();
    } catch { toast.error('সমস্যা হয়েছে'); }
  };
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <button onClick={() => { setShowAddForm(s => !s); setShowGenerator(false); }}
      ...
      {showAddForm && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border-2 border-gray-200 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-gray-700">ম্যানুয়াল সারি যোগ করুন</p>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={addRow} className="bg-gray-700 hover:bg-gray-800 text-white font-semibold px-8 py-2.5 rounded-xl transition-colors text-sm">যোগ করুন</button>
            <button onClick={() => setShowAddForm(false)} className="bg-white hover:bg-gray-50 text-gray-600 font-medium px-6 py-2.5 rounded-xl border-2 border-gray-200 transition-colors text-sm">বাতিল</button>
          </div>
        </div>
      )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm" style={{ minWidth: '1100px' }}>
              <colgroup>
                ...
                <col style={{width:80}} /><col style={{width:100}} /><col style={{width:130}} />
                <col style={{width:100}} /><col style={{width:100}} /><col style={{width:70}} />
                <col style={{width:80}} /><col style={{width:75}} /><col style={{width:90}} />
              </colgroup>
              <thead className="bg-gray-50 text-gray-500 text-xs">
                <tr>
                  {['#','ধরন','তারিখ','সময়','সাবজেক্ট','শিরোনাম','শিক্ষক','Zoom','ধরণ','স্থান','স্ট্যাটাস',''].map((h, i) => (
              ...
              <tbody>
                {outline.map(row => (
                  <OutlineRow key={row.id} row={row} teachers={teachers} zooms={zooms} onRefresh={loadOutline} onFeedback={setFeedbackRow} />
                ))}
              </tbody>
            </table>
          </div>
